...
type RateImpactClassifications = {
  [key: string]: number;
};

const findRateClassification = (rateImpactClassifications: RateImpactClassifications, percentage: number): string => {
  const sortedClassifications = Object.entries(rateImpactClassifications)
      .sort((a, b) => a[1] - b[1]); // Sort by values in ascending order

  if (percentage < sortedClassifications[0][1]) {
      return sortedClassifications[0][0]; // Return lowest classification if percentage is below the lowest
  }

  for (let i = 0; i < sortedClassifications.length - 1; i++) {
      if (percentage <= sortedClassifications[i + 1][1]) {
          return sortedClassifications[i][0];
      }
  }
...
const getUserLocalDateTime = (unixTimestamp: number, utcOffset: number) => {
  // Create a Date object from the Unix timestamp
  const dateTime = new Date(unixTimestamp * 1000);

  // Convert the local time to UTC (subtract the local timezone offset)
  const utcTime = new Date(dateTime.getTime() + dateTime.getTimezoneOffset() * 60 * 1000);

  // Convert the UTC offset to milliseconds
  const offsetMs = utcOffset * 60 * 60 * 1000;

  // Apply the offset to the current UTC time
  const userDateTime = new Date(utcTime.getTime() + offsetMs);
...
const isItSpecificTimeInUsersLocalTimezone = (unixTimestamp: number, utcOffset: number, specificTime: string) => {
  ...
  // Adjust the UTC time to the user's local time
  const userLocalTime = new Date(utcTime.getTime() + offsetMs);
